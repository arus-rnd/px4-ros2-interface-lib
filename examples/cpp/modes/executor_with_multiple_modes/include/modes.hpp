...
{
public:
  explicit FlightModeTestStart(rclcpp::Node & node) : ModeBase(node, Settings{kNameFirst, false})
  {
  ...
  void updateSetpoint(float dt_s) override
  {
    ...
    const Eigen::Vector3f do_not_regulate{NAN, NAN, -NAN};

    _trajectory_setpoint->update(do_not_regulate, velocity, do_not_regulate);
  }
...
{
public:
  explicit FlightModeTestSegment(rclcpp::Node & node) : ModeBase(node, Settings{kNameSecond, false})
  {
...
{
public:
  explicit FlightModeTestEnd(rclcpp::Node & node) : ModeBase(node, Settings{kNameThird, false})
  {
    _trajectory_setpoint = std::make_shared<px4_ros2::TrajectorySetpointType>(*this);
  }
